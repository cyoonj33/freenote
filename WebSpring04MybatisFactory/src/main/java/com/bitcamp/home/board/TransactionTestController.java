...
public class TransactionTestController {
 ...
 public ModelAndView transactionTest(BoardVO vo, HttpSession session,HttpServletRequest req) {
	 ...
	 //====트랜색션 구현을 위한 객체를 생성하고
	 DefaultTransactionDefinition def= new DefaultTransactionDefinition();
	 
	 //propagation_required :트랜색션이 있으면 그 상황에서 실행, 트랙색션이 없으면 새로운 트랙색션을 생성해서 실행
	 def.setPropagationBehavior(DefaultTransactionDefinition.PROPAGATION_REQUIRED);//트랜색션을 호출
