	...
	public void actionPerformed(ActionEvent ae) {
		...
		public void setNewTableList(List<MemberVO> lst) {
			...
			for(int i=0; i<lst.size(); i++) {
				...
				getJoin = joindateTf.getText();
				String birth = vo.getBirth().substring(0,10); 
				if(!getJoin.equals("") ||!getJoin.equals(null))  {
					join = vo.getJoindate();
		...
		//회원 신규 추가
		public void addMember(int idOK) {  
	 		idCheck = idTf.getText();
	 		int result = 0;
	 		MemberDAO dao = new MemberDAO();
	 		MemberVO vo;
	 		...
	 		if(idCheck.equals("")|| pwdTf.getText().equals("")|| nameTf.getText().equals("") 
	 			...
	 			if(idOK ==1) { //아이디가 중복이 아닐 경우
	 				if(joindateTf.getText()!="" || joindateTf.getText()!=null) { //가입일이 기입되어있다면
	 					...
	 					vo = new MemberVO(idTf.getText(), pwdTf.getText(), nameTf.getText(), telTf.getText(), birthdateTf.getText());
		...
		public void deleteMember() {
			...
			int result1 = dao1.checkMemberDel(id);
			 
			if(result1>0) {
		 		JOptionPane.showMessageDialog(this, "예약중인(결제 미완료) 내역이 있어 삭제가 불가합니다.");
		 	}else{  
