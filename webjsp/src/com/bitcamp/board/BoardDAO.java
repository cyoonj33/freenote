...
//게시판 글 등록
	public int insertBoard(BoardVO vo) {
		...
		try {
			...
			System.out.println("게시판 글 에러"+e.getMessage());
		}
		finally {
			closeDB();
	...
	public int totalRecord(String searchKey, String searchWord) {
		...
		try {
			...
			System.out.println("총레코드 수 구하기 에러 발생.."+e.getMessage());
	...
	//레코드 선택                        //현재페이지        한번 레코드수       총레코드수            총페이지           
	public List<BoardVO> selectRecord(int nowNum, int onePageRecord, int totalRecord, int totalPage ,String searchKey, String searchWord){
		System.out.println(nowNum+","+onePageRecord);
